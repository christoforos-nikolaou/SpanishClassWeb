## Clases de Español en Línea - Adriana

Este es el sitio web de nuestras clases de español. Aquí encontrarás toda la información que necesitas para seguir tu viaje de aprendizaje del español con el objetivo de presentarte a la certificación DELE C1.

En esta pagina encontrarás:
- El [calendario de clases](), con detalles sobre las cosas que aprendemos en cada sesión.
- Información sobre las tareas y ejercicios.
- Ejemplos de ensayos, cartas, articulos y trabajos escritos.
- Notas y cuadernos de gramática.
- Enlaces a los recursos auditivos que utilizamos en clase.
- 
- Enlaces a textos, materiales de estudio y recursos útiles.
- Recursos adicionales para el aprendizaje del español.
  

Adriana, tu profesora de español en línea. Ofrezco clases personalizadas para todos los niveles, desde principiantes hasta avanzados. Mis clases están diseñadas para ayudarte a mejorar tus habilidades de conversación, gramática y vocabulario de manera efectiva y divertida.